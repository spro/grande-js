...
class Model {
    ...
    static create_table_sql(drop) {
        ...
        const field_defs_sql = (0, fields_1.make_field_defs_sql)(field_defs);
    ...
    static async create(create_obj) {
        const defined_create_obj = (0, helpers_1.defined)(create_obj);
    ...
    // Search
    static async search(field_names, q, options = {}) {
        const where_clauses = field_names.map((field_name) => `${field_name} ilike $1`);
        ...
        q = '%' + q + '%';
        if (DEBUG)
            console.log('[search query]', select_query, [q]);
        const result = await this._pool.query(select_query, [q]);
        return result.rows.map(row => new this(row));
    }
    // Update
    static async update(id, update_obj) {
        const defined_update_obj = (0, helpers_1.defined)(update_obj);
    ...
    async add_related(relationship_name, items) {
        ...
        if (relationship_class.kind == 'forward') {
            ...
            field_values = (0, helpers_1.flatten)(items.map((item) => [from_id, item.id]));
        }
        else {
            const to_id = this.id;
            field_values = (0, helpers_1.flatten)(items.map((item) => [item.id, to_id]));
        }
